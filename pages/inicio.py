...
df_1 = data.df_Dados
df_2 = data.df_Historico
Saldo = data.Saldo['Saldo'].iloc[0]
dia = dt.datetime(2024, 1, 3)
nova_data = fc.mes_anterior(dia)
print(Saldo)

ultimo_dado = df_1['Data'].iloc[-1]

df_ultimo_mes = df_1[df_1['Data'] == ultimo_dado]
movimentacao_total = data.ultimo_aporte
...
resultado_inflacao_12_meses = (data.inflacao_12_meses['multiplicacao'].iloc[0])-1
resultado_12_meses = (data.rendimento_12_meses['multiplicacao'].iloc[0])-1
resultado_ano = data.rendimento_ano['multiplicacao'].iloc[0]-1
resultado_inflacao_ano = data.inflacao_ano['multiplicacao'].iloc[0]-1
movimentacao_media = data.media_aporte['Movimentacao_Total'].mean()
ganho_ano = data.ganho_ano['Ganho_total'].iloc[0]
...
layout = dbc.Container(
    ...
    children=[dbc.Row(
        children=[dbc.Col(
            children=[dbc.Card(
                children=[dbc.CardHeader('Inflação em 12 meses', className='paragrafo-personalizado'),
                          dbc.CardBody(html.H4(f"{resultado_inflacao_12_meses:.2%}",
        ...
        dbc.Row(
            children=[dbc.Col(
                children=[dbc.Card(
                    children=[dbc.CardHeader('Inflação no Ano', className='paragrafo-personalizado'),
                              dbc.CardBody(html.H4(f"{resultado_inflacao_ano:.2%}",
                                                   className='valor-personalizado'),
                                           className="d-flex justify-content-center")
                              ],
                    color="transparent",  # Cor de fundo
                ...
                dbc.Col(
                    children=[dbc.Card(
                        children=[dbc.CardHeader('Rendimento no Ano', className='paragrafo-personalizado'),
                                  dbc.CardBody(html.H4(f"{resultado_ano:.2%}",
                                                       className='valor-personalizado'),
                                               className="d-flex justify-content-center")
                                  ],
                        color="transparent",  # Cor de fundo
                ...
                dbc.Col(
                    children=[dbc.Card(
                        children=[dbc.CardHeader('Média de aportes no Ano', className='paragrafo-personalizado'),
                                  dbc.CardBody(html.H4(f"R$ {movimentacao_media:.2f}",
                                                       className='valor-personalizado'),
                                               className="d-flex justify-content-center")
                                  ],
                        color="transparent",  # Cor de fundo
                ...
                dbc.Col(children=[dbc.Card(
                    children=[dbc.CardHeader('Ganhos no Ano', className='paragrafo-personalizado'),
                              dbc.CardBody(html.H4(f"R$ {ganho_ano:.2f}", className='valor-personalizado'),
                                           className="d-flex justify-content-center")
                              ],
                    color="transparent",  # Cor de fundo
                    inverse=True,  # Texto branco
